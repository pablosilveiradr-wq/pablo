...
export const Road: React.FC<{ durationInFrames: number }> = ({
  durationInFrames,
}) => {
  ...
  const leadR = interpolate(t, [0, 2.4], [92, 26], { extrapolateLeft: "clamp" });
  const leadPos = onTrack(lead, leadR);

  const trail = [0, 1, 2].map((i) => {
    const s = interpolate(t, [0.9, 2.4], [-0.02 + i * 0.075, 0.06 + i * 0.075], {
      extrapolateLeft: "clamp",
    });
    const r = 32;
